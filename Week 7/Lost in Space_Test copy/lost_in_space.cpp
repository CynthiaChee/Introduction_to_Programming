...
point_2d power_mini_map_coordinate(const power_up_data &power_up)
{
    ...
    mini_map_x = (power_up_x - MIN_XY)/ 3000 *100 + 25;
...
void draw_hud (const game_data &game)
{
    ...
    //fuel information
    draw_bitmap("full", 200, 700, option_part_bmp(0, 0, (bitmap_width("full") * game.player.fuel_pct), bitmap_height("full"), option_to_screen()));
    //bonus bar
    draw_bitmap("empty_bar_transparent", 200, 740, option_to_screen());
    //bonus information
    draw_bitmap("orange_bar_lines", 200, 740, option_part_bmp(0, 0, (bitmap_width("orange_bar_lines") * game.player.bonus), bitmap_height("orange_bar_lines") * 0, option_to_screen()));
    //health parameter
    ...
    //player score
    //draw_text("SCORE", COLOR_WHITE);
    draw_text(to_string(game.player.score), COLOR_WHITE, 150, 675, option_to_screen());
    //draw_text("TIME REMAINING = : " + to_string(game.player.game_time), COLOR_BLACK, 0, 70, option_to_screen()),
    //player location
    draw_text("LOCATION: " + point_to_string(center_point(game.player.player_sprite)), COLOR_WHITE, 400, 675, option_to_screen());
